...
class SPADE_ResNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_0 = Convolution(spatial_dims = spatial_dims,
                                  in_channels = self.in_channels,
                                  out_channels = self.int_channels,
                                  act = None,
                                  norm = None,
                                  )
        self.conv_1 = Convolution(spatial_dims = spatial_dims,
        ...
        self.norm_0 = SPADE(label_nc=label_nc, norm_nc=self.in_channels, kernel_size=kernel_size,
                            spatial_dims=spatial_dims, hidden_channels=spade_intermediate_channels,
                            norm=norm)
        self.norm_1 = SPADE(label_nc=label_nc, norm_nc=self.int_channels, kernel_size=kernel_size,
                            spatial_dims=spatial_dims, hidden_channels=spade_intermediate_channels,
                            norm=norm)

        if self.learned_shortcut:
            self.conv_s = Convolution(spatial_dims = spatial_dims,
                                      in_channels = self.in_channels,
                                      out_channels = self.out_channels,
                                      act = None,
                                      norm = None,
            ...
            self.norm_s = SPADE(label_nc=label_nc, norm_nc=self.in_channels, kernel_size=kernel_size,
                                spatial_dims=spatial_dims, hidden_channels=spade_intermediate_channels,
                                norm=norm)
    ...
    def __init__(self,
        ...
        for s_ind, s_ in enumerate(input_shape):
            if s_ / (2 ** len(num_channels)) != s_ // (2 ** len(num_channels)):
                raise ValueError("Each dimension of your input must be divisible by 2 ** (autoencoder depth)."
        ...
        self.fc_mu = nn.Linear(in_features=np.prod(self.latent_spatial_shape) * self.num_channels[-1],
                                out_features=self.z_dim)
        self.fc_var = nn.Linear(in_features=np.prod(self.latent_spatial_shape) * self.num_channels[-1],
                                out_features=self.z_dim)
...
class SPADE_Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        for s_ind, s_ in enumerate(input_shape):
            if s_ / (2 ** len(num_channels)) != s_ // (2 ** len(num_channels)):
                raise ValueError("Each dimension of your input must be divisible by 2 ** (autoencoder depth)."
                                 "The shape in position %d, %d is not divisible by %d. " % (
                                 s_ind, s_, len(num_channels)))
        ...
        for ch_ind, ch_value in enumerate(num_channels[:-1]):
            blocks.append(SPADE_ResNetBlock(spatial_dims=spatial_dims,
        ...
        self.last_conv = Convolution(spatial_dims=spatial_dims,
    ...
    def forward(self, seg, z: torch.Tensor = None):
        ...
        if self.is_gan:
            ...
            if z is None:
                z = torch.randn(seg.size(0), self.opt.z_dim,
