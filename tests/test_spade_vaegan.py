...
from networks.nets.spade_network import SPADE_Net
import numpy as np
...
def create_Semantic_Data(shape:list, semantic_regions:int):
    ...
    for i in range(1, semantic_regions):
        ...
        start_point = [np.random.choice(list(range(shape[ind]-shape_square[ind])))
                       for ind, i in enumerate(shape)]
        if len(shape) == 2:
            out_label[start_point[0]:(start_point[0]+shape_square[0]),
                      start_point[1]:(start_point[1]+shape_square[1])] = i
            base_intensity = torch.ones(shape_square) * np.random.randn()
            out_image[start_point[0]:(start_point[0] + shape_square[0]),
            start_point[1]:(start_point[1] + shape_square[1])] = base_intensity + \
                                                           torch.randn(shape_square)*0.1
        elif len(shape) == 3:
            out_label[start_point[0]:(start_point[0]+shape_square[0]),
                      start_point[1]:(start_point[1]+shape_square[1]),
                      start_point[2]:(start_point[2] + shape_square[2])] = i
            base_intensity = torch.ones(shape_square) * np.random.randn()
            out_image[start_point[0]:(start_point[0]+shape_square[0]),
                      start_point[1]:(start_point[1]+shape_square[1]),
                      start_point[2]:(start_point[2] + shape_square[2])] = base_intensity + \
                                                           torch.randn(shape_square)*0.1
        else:
...
class TestDiffusionModelUNet2D(unittest.TestCase):
    ...
    def test_forward_2d(self, input_param):
        ...
        with eval_mode(net):
            ...
            self.assertEqual(False, True in torch.isnan(out) or True in torch.isinf(out)
                             or True in torch.isinf(kld) or True in torch.isinf(kld))
